	...
	public int doStartTag() throws JspException {
		try {
			...
			for (KvEntity kvEntity : items) {
				result.append("<input type=\"radio\" id=\"").append(getId()).append(kvEntity.getCode()).append("\" name=\"").append(getName()).append("\" class=\"").append(getCssCls()).append("\" ").append(getExp());
				result.append(" value=\"").append(kvEntity.getCode()).append("\"");
				if(FrameStringUtil.isNotEmpty(value) && value.equals(kvEntity.getCode())) {
					result.append(" checked=\"checked\"");
				}
				result.append(" title=\"").append(kvEntity.getValue()).append("\" /> ");
			}
